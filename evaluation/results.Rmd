...
curdata <- curdata %>% mutate(question = case_match(question, 
                                                    ...
                                                    "QU01_02" ~ "I found the system unnecessarily complex. (reversed to: The system was found simple.)",
                                                    "QU01_03" ~ "I thought the system was easy to use.",
                                                    "QU01_04" ~ "I think that I would need the support of a technical person to be able to use this system. (reversed to: No support was needed.)",
                                                    "QU01_05" ~ "I found the various functions in this system were well integrated.",
                                                    "QU01_06" ~ "I thought there was too much inconsistency in this system. (reversed to: The system was consistent.)",
                                                    "QU01_07" ~ "I would imagine that most people would learn to use this system very quickly.",
                                                    "QU01_08" ~ "I found the system very cumbersome to use.  (reversed to: The system was easy to use.)",
                                                    "QU01_09" ~ "I felt very confident using the system.",
                                                    "QU01_10" ~ "I needed to learn a lot of things before I could get going with this system (reversed to: The system was intuitive.)"))
...
ggplot(curdata, aes(x="", y=nr, fill=answer)) + 
 ...
 # geom_label(data=curdata %>% filter(answer %in% c(3,4,5)), aes(y=nr*1.5, label=nr))+
  ...
  facet_wrap("question", nrow=10, labeller = label_wrap_gen(width=85), scales="free")+
  theme(legend.justification ="center")
  ggsave("images/SUS.jpg", height=9.5, width=6.5)
...
curdata <- data %>% 
  select(columns) %>%
  #filter(across(columns, ~ .x > 0)) %>%
  mutate(across(columns, ~ ifelse(.x < 0, 3, .x))) %>%
  mutate(QU01_01 = as.integer(QU01_01), QU01_02 = as.integer(QU01_02), QU01_03 = as.integer(QU01_03), QU01_04 = as.integer(QU01_04), QU01_05 = as.integer(QU01_05), QU01_06 = as.integer(QU01_06), QU01_07 = as.integer(QU01_07), QU01_08 = as.integer(QU01_08), QU01_09 = as.integer(QU01_09), QU01_10 = as.integer(QU01_10), .keep="unused")
curdata <- curdata %>%
  rowwise() %>%
